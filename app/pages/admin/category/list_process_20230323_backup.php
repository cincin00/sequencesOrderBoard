<?php
  ...
  if (validSingleData($params, 'mode')) {
      if ($params['mode'] === 'create_node') {
          ...
          $parentNodeDepth = ($params['parent_code'] !== 'root' ? strlen($params['parent_code']) : 1);
          $currentDepth = ($params['parent_code'] !== 'root' ? ($parentNodeDepth < 4 ? (strlen($params['parent_code']) + 1) : 4) : 2);
          // 신규 카테고리 코드 조회
          $newCategoryCode = setCategoryCode($currentDepth, $params['depth_order'], $params['parent_code']);
        //   var_dump('parentNodeDepth:::'.$parentNodeDepth);
        //   var_dump('currentDepth:::'.$currentDepth);
        //   var_dump('newCategoryCode:::'.$newCategoryCode);
        //   dd('EOF');
          /** Node 정렬의 재귀함수 */
          changeCategoryNode($params['category_code'], $newCategoryCode);
          $response['data'] = getCategoryForAdminCategoryList();
          echo $response['data'];
          exit;
