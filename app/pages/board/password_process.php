<?php
    ...
    $msg = $href = $pagePrefix = '';
    $mypostRef = checkpage($_SERVER['HTTP_REFERER'], 'mypost');
    if($mypostRef){
        $pagePrefix = 'mypost_';
    }
    ...
    if (empty($postId) === true) {
        $msg = '존재하지 않는 게시글입니다.';
        $href = BOARD_DIR.'/'.$pagePrefix.'list.php';
    } elseif (empty($mode) === true) {
        $msg = '잘못된 요청입니다.';
        $href = BOARD_DIR.'/'.$pagePrefix.'view.php?board_id='.$boardId.'&id='.$postId;
    ...
    if ($mode === 'update') {
        ...
        if ($isReply) {
            commonMoveAlert('답글이 있는 경우 수정 할 수 없습니다.', BOARD_DIR.'/'.$pagePrefix.'view.php?board_id='.$boardId.'&id='.$postId);
        }
    } else {
        // 답글 검사
        $isReply = havePostReplay((int)$postData['id']);
        ...
        if ($isReply) {
            commonMoveAlert('답글이 있는 경우 삭제 할 수 없습니다.', BOARD_DIR.'/'.$pagePrefix.'view.php?board_id='.$boardId.'&id='.$postId);
        }
    }
    ...
    if ($mode === 'update') {        
        ...
        // 수정 페이지 호출
        include_once(BASEPATH.'/app/pages/board/'.$pagePrefix.'modify.php');
    } elseif ($mode === 'delete') {
        // 삭제 처리
        ...
        if ($queryResult) {
            ...
            $msg = '게시글 삭제가 실패했습니다.';
            $href = BOARD_DIR.'/'.$pagePrefix.'view.php?id='.$id;
        }
        commonMoveAlert($msg, $href);
    } else {
        commonMoveAlert('잘못된 요청입니다.', BOARD_DIR.'/'.$pagePrefix.'list.php');
    }
