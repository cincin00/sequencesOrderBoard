<?php
    ...
    if (empty($msg) === false) {
        commonMoveAlert($msg, BOARD_DIR.'/'.$pagePrefix.'modify.php');
    }
    ...
    if ($postData['member_id']) {
        ...
        if ((int)$memberId !== $postData['member_id']) {
            // 회원일떄, 본인 게시글 검증
            commonMoveAlert('게시글 수정이 권한이 없습니다.', BOARD_DIR.'/'.$pagePrefix.'view.php?board_id='.$params['board_id'].'&id='.$postId);
        }
    } else {
        // 비회원 게시글
        if ($postData['password'] !== $password) {
            // 비회원일때, 비밀번호 검증
            commonMoveAlert('게시글 수정이 실패했습니다.', BOARD_DIR.'/'.$pagePrefix.'view.php?board_id='.$params['board_id'].'&id='.$postId);
        }
    }
    ...
    if ($result) {
        commonMoveAlert('게시글이 수정되었습니다.', BOARD_DIR.'/'.$pagePrefix.'view.php?board_id='.$params['board_id'].'&id='.$postId);
    } else {
        commonMoveAlert('게시글 수정이 실패했습니다.', BOARD_DIR.'/'.$pagePrefix.'view.php?board_id='.$params['board_id'].'&id='.$postId);
    }
